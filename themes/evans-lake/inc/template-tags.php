 ...
 */
  ...
  */
 function evans_lake_comment_count() {
  ...
  */
 function evans_lake_entry_footer() {
 	// Hide category and tag text for pages.
 	if ( 'post' === get_post_type() ) {
 		/* translators: used between list items, there is a space after the comma */
 		$categories_list = get_the_category_list( esc_html( ', ' ) );
 		if ( $categories_list && evans_lake_categorized_blog() ) {
  ...
  * @return bool
  */
 function evans_lake_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'evans_lake_categories' ) ) ) {
 		...
 		set_transient( 'evans_lake_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so evans_lake_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so evans_lake_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in evans_lake_categorized_blog.
  */
 function evans_lake_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'evans_lake_categories' );
 }
 add_action( 'edit_category', 'evans_lake_category_transient_flusher' );
 add_action( 'save_post',     'evans_lake_category_transient_flusher' );
  ...
  */
 function evans_lake_comment_list( $comment, $args, $depth ) {
  ...
  */
 function evans_lake_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
